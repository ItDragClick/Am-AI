...
| `CraftPlanner` | `ai/CraftPlanner.java` | Progressive crafting engine (wooden/stone/iron pickaxe). Restart-safe: re-derives the next step from the inventory every tick (gather wood → planks/sticks via 2x2 InventoryMenu clicks (grid 1-4, result 0) → crafting table → place+open it → 3x3 recipes via CraftingMenu clicks (grid 1-9) → mine stone → stone pick → mine iron → craft+place furnace → smelt raw_iron (FurnaceMenu 0=input/1=fuel/2=result) → iron pick). 60s per-step timeout aborts. `mine <pickaxe>` requests auto-route here. UNTESTED in-game — container click choreography is the risk area. |
